...
function CoverImage({ storageId }: { storageId: string }) {
  const isStorageId = !storageId.startsWith("http");
  const resolved = useQuery(api.files.getImageUrl, isStorageId ? { storageId } : "skip");
  const url = isStorageId ? resolved : storageId;
  if (!url) return null;
  return (
