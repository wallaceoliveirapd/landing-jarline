...
export function ChatWidget({ config, initialMessage, isOpen: externalIsOpen, onOpenChange }: ChatWidgetProps) {
  ...
  if (!isOpen) {
    return (
      <button
        ...
        className="fixed bottom-6 right-6 z-[700] bg-[#A27E63] text-white p-4 rounded-full shadow-2xl hover:scale-110 transition-transform cursor-pointer"
  ...
  return (
    <div className="fixed inset-0 md:inset-auto md:bottom-6 md:right-6 z-[700] md:w-[380px] md:h-[520px] w-full h-[100dvh] md:rounded-3xl bg-white shadow-2xl flex flex-col overflow-hidden animate-in slide-in-from-bottom-4 duration-300">
      {/* Header */}
      <div className="bg-primary px-5 py-4 flex items-center justify-between shrink-0">
        ...
        <button
          onClick={handleClose}
          style={{ touchAction: 'manipulation' }}
          className="text-white/80 hover:text-white transition-colors cursor-pointer p-2 -mr-2"
        >
      ...
      {/* Messages */}
      <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-zinc-50" style={{ overscrollBehavior: 'contain' }}>
        {messages.length === 0 && (
          <div className="text-center py-8">
            ...
            <div className="flex flex-col gap-2">
              <button
                ...
                style={{ touchAction: 'manipulation' }}
                className="text-left text-sm bg-white p-3 rounded-xl border border-zinc-100 hover:border-primary/30 hover:shadow-md transition-all cursor-pointer"
              >
      ...
      {/* Input */}
      <div className="p-3 bg-white border-t border-zinc-100 shrink-0" style={{ paddingBottom: 'calc(0.75rem + env(safe-area-inset-bottom))' }}>
        <div className="relative">
          <textarea
            ref={inputRef}
            value={inputValue}
            ...
            rows={1}
            className="w-full px-4 py-3 pr-12 bg-zinc-50 border border-zinc-100 rounded-2xl resize-none focus:outline-none focus:border-primary/30 focus:bg-white transition-all"
            style={{ fontSize: '16px' }}
          />
          <button
            onClick={handleSend}
